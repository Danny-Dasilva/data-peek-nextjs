...
import {
  ...
  SheetHeader,
  SheetTitle
} from '@/components/sql/ui/sheet'
import { useConnectionStore, type Connection } from '@/stores'
import { PostgreSQLIcon, MySQLIcon, MSSQLIcon } from '@/components/sql/database-icons'
...
function parseMSSQLConnectionString(connectionString: string): {
  ...
  password: string
  ssl: boolean
  mssqlOptions?: import('@data-peek/shared').MSSQLConnectionOptions
} | null {
  try {
    ...
    // Build MSSQL options
    const mssqlOptions: import('@data-peek/shared').MSSQLConnectionOptions = {}
...
export function AddConnectionDialog({
  ...
  const [mssqlOptions, setMssqlOptions] = useState<
    import('@data-peek/shared').MSSQLConnectionOptions | undefined
  >(undefined)
  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      const result = await api.db.connect(config)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isEditMode) {
        ...
        const result = await api.connections.add(config)
