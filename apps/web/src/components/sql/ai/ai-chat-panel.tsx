...
export function AIChatPanel({
  ...
  React.useEffect(() => {
    ...
    const loadSessions = async () => {
      try {
        const response = await api.ai.getSessions(connectionId)
        if (response.success && response.data) {
          ...
          if (response.data.length > 0) {
            ...
            const createResponse = await api.ai.createSession(connectionId)
  ...
  React.useEffect(() => {
    ...
    const saveSession = async () => {
      try {
        ...
        const response = await api.ai.updateSession(connectionId, currentSessionId, {
  ...
  const handleNewSession = async () => {
    ...
    try {
      const response = await api.ai.createSession(connection.id)
      if (response.success && response.data) {
  ...
  const handleDeleteSession = async (sessionId: string) => {
    if (!connection?.id) return
    try {
      const response = await api.ai.deleteSession(connection.id, sessionId)
      if (response.success) {
  ...
  const handleSaveTitle = async () => {
    ...
    try {
      const response = await api.ai.updateSession(connection.id, editingSessionId, {
        title: editingTitle
      })
      if (response.success && response.data) {
  ...
  const handleClearChat = async () => {
    ...
    if (connection?.id && currentSessionId) {
      try {
        await api.ai.updateSession(connection.id, currentSessionId, { messages: [] })
      } catch (err) {
