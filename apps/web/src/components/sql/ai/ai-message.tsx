...
import { User, Sparkles, Copy, Check, AlertTriangle, Loader2, Code, ChevronDown, ChevronUp } from 'lucide-react'
...
export function AIMessage({ message, onOpenInTab, connection, schemas = [] }: AIMessageProps) {
  ...
  const handleExecuteInline = async (sql: string) => {
    ...
    try {
      const response = await api.db.query(connection, sql)
  ...
  React.useEffect(() => {
    const chartSql = message.responseData?.type === 'chart'
      ? (message.responseData as AIChartData).sql
      : null

    if (chartSql && connection && !chartFetchedRef.current) {
      chartFetchedRef.current = true
      const fetchChartData = async () => {
        setChartLoading(true)
        try {
          const response = await api.db.query(connection, chartSql)
          if (response.success && response.data) {
  ...
  // Use ref to prevent infinite fetch loops - only fetch once per message
  React.useEffect(() => {
    const metricSql = message.responseData?.type === 'metric'
      ? (message.responseData as AIMetricData).sql
      : null

    if (metricSql && connection && !metricFetchedRef.current) {
      metricFetchedRef.current = true
      const fetchMetricData = async () => {
        setMetricLoading(true)
        try {
          const response = await api.db.query(connection, metricSql)
          if (response.success && response.data) {
            const data = response.data as { rows: Record<string, unknown>[] }
        ...
        if (chartData && chartData.length > 0) {
          ...
          return (
            <div className="mt-3 space-y-2">
              ...
              {chartResponseData.sql && (
                <SQLCollapsible sql={chartResponseData.sql} />
              )}
            </div>
          )
        }
      ...
      case 'metric': {
        ...
        return (
          <div className="mt-3 space-y-2">
            ...
            {metricResponseData.sql && (
              <SQLCollapsible sql={metricResponseData.sql} />
            )}
          </div>
        )
      }
