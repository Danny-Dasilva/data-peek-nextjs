...
export function BenchmarkButton({ tabId, query, connection, disabled }: BenchmarkButtonProps) {
  const isRunning = useTelemetryStore((s) => s.benchmarkRunning.get(tabId) ?? false)
  ...
  const handleBenchmark = async (runCount: number) => {
    if (!query.trim() || !connection) return

    setBenchmarkRunning(tabId, true)
    setShowTelemetryPanel(true)

    try {
      const response = await fetch('/api/sql/db/benchmark', {
        method: 'POST',
      ...
      if (result.success && result.data) {
        setTabBenchmark(tabId, result.data)
      }
    } catch (error) {
      console.error('Benchmark failed:', error)
    } finally {
      setBenchmarkRunning(tabId, false)
    }
  }
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                ...
                disabled={isRunning || disabled || !connection}
              >
                {isRunning ? (
                  <>
