...
import { TableBody, TableCell, TableHead, TableHeader, TableRow } from '@/components/sql/ui/table'
...
import { EditableCell } from '@/components/sql/editable-cell'
import { EditToolbar } from '@/components/sql/edit-toolbar'
import { SqlPreviewModal } from '@/components/sql/sql-preview-modal'
import { JsonCellValue } from '@/components/sql/json-cell-value'
import { FKCellValue } from '@/components/sql/fk-cell-value'
import { AddRowSheet, type ForeignKeyValue } from '@/components/sql/add-row-sheet'
...
export function EditableDataTable<TData extends Record<string, unknown>>({
      ...
      try {
        ...
        await Promise.all(
          fkColumns.map(async (col) => {
            ...
            try {
              const result = await api.db.query(connection, query)
  ...
  const handlePreviewSql = async () => {
    ...
    try {
      const response = await api.db.previewSql(batch)
  ...
  const handleConfirmCommit = async () => {
    ...
    try {
      const response = await api.db.execute(connection, batch)
