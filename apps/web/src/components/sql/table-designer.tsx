...
import { Checkbox } from '@/components/sql/ui/checkbox'
import {
  Select,
  SelectContent,
  SelectItem,
...
import { Collapsible, CollapsibleContent, CollapsibleTrigger } from '@/components/sql/ui/collapsible'
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
import { ConstraintEditor } from '@/components/sql/constraint-editor'
import { IndexEditor } from '@/components/sql/index-editor'
...
export function TableDesigner({ tabId }: TableDesignerProps) {
  ...
  useEffect(() => {
    const loadExistingTable = async () => {
      ...
      try {
        const response = await api.ddl.getTableDDL(
          tabConnection,
          tab.schemaName,
          tab.tableName
        )
  ...
  const handlePreviewSQL = useCallback(async () => {
    ...
    try {
      const response = await api.ddl.previewDDL(definition)
      if (response.success && response.data) {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      const response = await api.ddl.createTable(tabConnection, definition)
