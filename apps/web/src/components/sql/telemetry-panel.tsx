...
import { useTelemetryStore, type TelemetryViewMode, type PercentileOption } from '@/stores/telemetry-store'

interface TelemetryPanelProps {
  tabId: string
}

interface TelemetryPanelInnerProps {
  ...
  onSelectPercentile: (p: PercentileOption) => void
...
/**
 * Wrapper component that uses the telemetry store
 */
export function TelemetryPanel({ tabId }: TelemetryPanelProps) {
  const telemetry = useTelemetryStore((s) => s.tabTelemetry.get(tabId))
  const benchmark = useTelemetryStore((s) => s.tabBenchmark.get(tabId))
      ...
      {benchmark && (
        <div className="px-4 py-2.5 border-t border-border/40 bg-muted/20">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <StatBadge label="o" value={formatDuration(benchmark.stats.stdDev)} />
          </div>
        </div>
      )}
