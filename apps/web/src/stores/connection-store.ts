...
export const useConnectionStore = create<ConnectionState>((set, get) => ({
  ...
  updateConnection: async (id, updates) => {
    ...
    try {
      const result = await api.connections.update(updated)
      if (result.success) {
        set((state) => ({
  ...
  fetchSchemas: async (connectionId?: string, forceRefresh?: boolean) => {
    ...
    try {
      ...
      const schemasResult = await api.db.schemas(connection, forceRefresh)
  ...
  refreshSchemasInBackground: async (connectionId?: string) => {
    ...
    try {
      const schemasResult = await api.db.schemas(connection, true) // Force refresh
