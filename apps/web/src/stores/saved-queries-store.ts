...
export const useSavedQueryStore = create<SavedQueryState>((set, get) => ({
  ...
  addSavedQuery: async (queryData) => {
    ...
    try {
      const result = await api.savedQueries.add(newQuery)
      if (result.success && result.data) {
        set((state) => ({
  ...
  updateSavedQuery: async (id, updates) => {
    try {
      const result = await api.savedQueries.update(id, updates)
      if (result.success && result.data) {
        set((state) => ({
  ...
  deleteSavedQuery: async (id) => {
    try {
      const result = await api.savedQueries.delete(id)
      if (result.success) {
        set((state) => ({
  ...
  incrementUsage: async (id) => {
    try {
      const result = await api.savedQueries.incrementUsage(id)
      if (result.success && result.data) {
        set((state) => ({
          savedQueries: state.savedQueries.map((q) => (q.id === id ? result.data! : q))
        }))
